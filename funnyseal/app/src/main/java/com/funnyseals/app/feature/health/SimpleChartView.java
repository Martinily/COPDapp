...
public class SimpleChartView extends View {
    ...
    protected void onDraw (Canvas canvas) {
        ...
        for (int j = 0; j < 6; j++) {
            ...
            if (j == 0 | j == 5)
                continue;
            canvas.drawLine(Xstart + Xlength, Ystart + j * 80, Xstart + Xlength + 10, Ystart + j
                    * 80, paint2);
        }
        for (int i = 1; i <= 7; i++) {
            canvas.drawLine(Xstart + i * 100, Ystart + Ylength, Xstart + i * 100, Ystart +
                    Ylength - 10, paint1);
        }
        //画小箭头
            ...
            if (data.size() >= 7) {
                for (int i = data.size() - 7; i < data.size() - 1; i++) {
                    if (data.get(i) == 0 | data.get(i + 1) == 0)
                        continue;
                    canvas.drawLine(Xstart + (i + 1) * 100, Ystart + Ylength - (data.get(i) -
                            1000) / 10, Xstart + (i + 2) * 100, Ystart + Ylength - (data.get(i +
                            1) - 1000) / 10, paint);
                }
            } else {
        ...
        if (data2.size() > 1) {
            if (data2.size() >= 7) {
                for (int i = data2.size() - 7; i < data2.size() - 1; i++) {
                    if (data2.get(i) == 0 | data2.get(i + 1) == 0)
                        continue;
                    canvas.drawLine(Xstart + (i + 1) * 100, Ystart + Ylength - (data2.get(i) -
                            50) * 8, Xstart + (i + 2) * 100, Ystart + Ylength - (data2.get(i + 1)
                            - 50) * 8, paint2);
                }
            } else {
